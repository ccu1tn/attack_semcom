...
from models_channel_inversion import sem_enc_model, chan_enc_model, Chan_Model, chan_dec_model, sem_dec_model
tf.config.run_functions_eagerly(True)

success_rate_metric = tf.keras.metrics.BinaryAccuracy()
...
def parse_args():
    ...
    parser.add_argument("--trainset_tfrecords_path", type=str, default="./output_tfrecords_1000audio/trainset.tfrecords", 
                        help="tfrecords path of trainset.") #./output_tfrecords_files_none_attack/trainset.tfrecords
    parser.add_argument("--validset_tfrecords_path", type=str, default="./output_tfrecords_1000audio/trainset.tfrecords",
...
# Modify the train_step function to include the constraints
if __name__ == "__main__":
    ...
    def train_step(input_data, std):
        lambda_lower = tf.Variable(tf.zeros_like(input_data), trainable=True)
        lambda_upper = tf.Variable(tf.zeros_like(input_data), trainable=True)
        std = tf.cast(std, dtype=tf.float32)
        max_amplitude = np.max(np.abs(input_data))
        threshold_percentage = 0.001  # can ddjust this value
        threshold_value = max_amplitude * threshold_percentage
        with tf.GradientTape() as tape:
            ...
            # success_mask = tf.math.less(tf.abs(input_data - _output), threshold_value)
            # success_rate_metric.update_state(success_mask, _output)
            mse = mse_loss(input_data, _output)
            a = -0.1
            b = 0.1
            # Constraints
            lower_bound_violation = tf.maximum(a - _output, 0)
            upper_bound_violation = tf.maximum(_output - b, 0)
            
            # KKT Lagrangian
            kkt_loss = mse + tf.reduce_sum(lambda_lower * lower_bound_violation) + tf.reduce_sum(lambda_upper * upper_bound_violation)
            
            # Update Lagrange multipliers
            lambda_lower.assign_add(lower_bound_violation)
            lambda_upper.assign_add(upper_bound_violation)
    ...
    common_dir = "./"
    saved_model = common_dir + "saved_model_original_mseKKT_inver/"
    
    # create files to save train loss
    train_loss_dir = common_dir + "train_original_mseKKT_inver/"   
    ...
    valid_loss_dir = common_dir + "valid_original_mseKKT_inver/"   
    ...
    for epoch in range(args.num_epochs):
        ...
        success_rate_value = success_rate_metric.result().numpy()
        #print(f'Training Success Rate after epoch {epoch + 1}: {success_rate_value}')
        success_rate_metric.reset_states()
        log = "train epoch {}/{}, train_loss = {:.06f}, Accr = {:.03f}, time = {:.06f}"
        print(log.format(epoch + 1, args.num_epochs, train_loss, success_rate_value, time.time() - start))
        ##########################    valid    ##########################
        # read .tfrecords file
