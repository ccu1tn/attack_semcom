# -*- coding: utf-8 -*-
"""
Created on Sat Mar  5 13:36:35 2024
...
def SEResNet(inputs, out_dim, name):
    ...
    #print('se',SEResNet_output.shape)
...
class Chan_Model(object):  
    ...
    def __call__(self, _input, std):
        ...
        #model = YourModel(num_classes=10)  # Replace 10 with the number of classes in your model

        # Initialize MRPPAttack instance
        epsilon_acc = 0.01
        Pmax = 1
        input_data = np.random.randn(10, 32, 32, 1)
        ...
        attack = MRPPAttack(epsilon_acc, Pmax) 
        #x_complex_noise = tf.complex(tf.math.real(attack) + noise[..., 0], tf.math.imag(x_complex) + noise[..., 1])
        noise1= attack.channel_inversion_attack(input_data) 
        #noise = compute_perturbation()
        x_complex_noise = x_complex + noise1
        noise = np.random.randn(*x_complex_noise.shape, 2)
        n_real = noise[:, :, :, 0]
        n_imag = noise[:, :, :, 1]
        n_complex = tf.dtypes.complex(real=n_real, imag=n_imag)
        #convert complex128 to complex64
        n_complex = tf.cast(n_complex, tf.complex64)
